...
// POST assign subject to advisory class
...
router.get('/faculty/statistics', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Log request information to diagnose the issue
    console.log("Faculty statistics request received");
    ...
    if (!facultyID) {
      console.error("No facultyID found in token:", req.user);
      return res.status(400).json({ 
        success: false, 
    ...
    console.log(`Processing statistics for facultyID: ${facultyID}`);
    
    // Get active school year
    const [schoolYearResults] = await db.query("SELECT school_yearID FROM schoolyear WHERE status = 1 LIMIT 1");
    ...
    const activeYearID = schoolYearResults[0].school_yearID;
    console.log(`Active school year ID: ${activeYearID}`);

    // Count students assigned to this faculty
    const [studentResults] = await db.query(`
      SELECT COUNT(DISTINCT sc.StudentID) AS students
      FROM assignsubject a
      JOIN advisory adv ON a.advisoryID = adv.advisoryID
      JOIN student_classes sc ON sc.advisoryID = adv.advisoryID
      WHERE a.FacultyID = ? AND a.yearID = ?
    `, [facultyID, activeYearID]);
    
    // Count subject classes assigned to this faculty
    const [classResults] = await db.query(`
      SELECT COUNT(*) AS subjectClasses
      FROM assignsubject
      WHERE FacultyID = ? AND yearID = ?
    `, [facultyID, activeYearID]);
    
    // Count unfinished grades (validation requests with status 0)
    const [unfinishedResults] = await db.query(`
      SELECT COUNT(*) AS unfinishedGrades
      FROM validation_request
      WHERE facultyID = ? AND statusID = 0
    `, [facultyID]);
    
    // Count finished grades (validation requests with status 1)
    const [finishedResults] = await db.query(`
      SELECT COUNT(*) AS finishedGrades
      FROM validation_request
      WHERE facultyID = ? AND statusID = 1
    `, [facultyID]);
    
    // Build the statistics object
    const statistics = {
      students: studentResults[0]?.students || 0,
      subjectClasses: classResults[0]?.subjectClasses || 0,
      unfinishedGrades: unfinishedResults[0]?.unfinishedGrades || 0,
      finishedGrades: finishedResults[0]?.finishedGrades || 0
    };
    
    console.log("Faculty statistics prepared:", statistics);
