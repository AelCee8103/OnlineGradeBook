...
const ValidationRequest = () => {
  ...
  const fetchRequests = useCallback(async () => {
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to fetch validation requests"
      );
    } finally {
      setLoading(false);
    }
      ...
      if (response.data.success) {
        ...
        if (socket && request) {
          ...
          socket.emit("validationResponse", {
            ...
            status: action,
            message: `Your grade validation request for Grade ${
              request.Grade
            } - ${request.Section} has been ${
          ...
          toast.custom(
            <div
              className={
                action === "approve"
                  ? "bg-green-600 text-white p-4 rounded shadow-lg"
                  : "bg-red-600 text-white p-4 rounded shadow-lg"
              }
            >
  ...
  // Only show pending requests (statusID === 0)
  const filteredRequests = requests
    .filter((request) => request.statusID === 0)
    .filter(
      (request) =>
        request.facultyName.toLowerCase().includes(searchTerm.toLowerCase()) ||
        `Grade ${request.Grade} - ${request.Section}`
          .toLowerCase()
...
// Add after your other admin routes
