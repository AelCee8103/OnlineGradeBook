...
router.get("/faculty/check-pending-request/:advisoryID", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    const request = validationRequests[0];
    
    // Map database status values to the same values used by socket events
    let status;
    if (request.status === 'pending') {
      status = 'pending';
    } else if (request.status === 'approved') {
      status = 'approve'; // Match socket event status value
    } else if (request.status === 'rejected') {
      status = 'reject'; // Match socket event status value
    }
    ...
    console.error('Error checking pending validation request:', error);
    res.status(500).json({
      success: false,
